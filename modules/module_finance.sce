...
global FIN_AXES FIN_MODE
global FIN_P FIN_R FIN_N FIN_EMIRES
global FIN_INV FIN_YRS FIN_RA FIN_RB
...
function finance_open()
    ...
    // ---- Investment Comparator panel (built in same space, hidden initially) ----
    ...
    FIN_AXES = a; FIN_MODE = mode;
    FIN_P = p_edit; FIN_R = r_edit; FIN_N = n_edit; FIN_EMIRES = emi_result;
    FIN_INV = inv_edit; FIN_YRS = yrs_edit; FIN_RA = ra_edit; FIN_RB = rb_edit;

    FIN_EMI_GROUP = list(p_lbl, p_edit, r_lbl, r_edit, n_lbl, n_edit, calc_btn, emi_result);
    FIN_INV_GROUP = list(inv_lbl, inv_edit, yrs_lbl, yrs_edit, ra_lbl, ra_edit, rb_lbl, rb_edit, inv_btn);
...
function finance_switch()
    global FIN_MODE FIN_EMI_GROUP FIN_INV_GROUP
    m = FIN_MODE.value;
...
function finance_calc_emi()
    global FIN_P FIN_R FIN_N FIN_EMIRES FIN_AXES
    ...
    sca(FIN_AXES);
    cla();
    plot2d(1:n, balances, style = color("lime"));
    xtitle('Outstanding Loan Balance Over Time', 'Month', 'Balance');

    FIN_EMIRES.string = msprintf('EMI: %.2f\nTotal Payment: %.2f\nTotal Interest: %.2f', emi, total, interest);
endfunction

function finance_calc_invest()
    global FIN_INV FIN_YRS FIN_RA FIN_RB FIN_AXES
    ...
    sca(FIN_AXES);
    cla();
    plot2d(1:n, valA, style = color("cyan"));
    plot2d(1:n, valB, style = color("magenta"));
    xtitle('Investment Growth: Scheme A (cyan) vs Scheme B (magenta)', 'Month', 'Value');
endfunction
