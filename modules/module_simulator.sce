...
global SIM_M SIM_K SIM_C SIM_X0 SIM_AXES SIM_INFO
...
function xd = sim_ode(t, x)
    // ODE right-hand side for the spring-mass-damper system.
    // Uses globals for m/k/c since Scilab's ode() calls f(t,x) only.
    global SIM_ODE_M SIM_ODE_K SIM_ODE_C
    xd = zeros(2, 1);
    xd(1) = x(2);
...
function sim_update()
    ...
    sca(SIM_AXES);
    cla();
    plot2d(t, x(1, :), style = color("cyan"));
    xtitle('Displacement Response', 'Time (s)', 'Displacement (m)');

    SIM_INFO.string = msprintf('Natural Freq wn: %.3f rad/s\nDamping Ratio zeta: %.3f', wn, zeta);
endfunction

function sim_animate()
    global SIM_M SIM_K SIM_C SIM_X0 SIM_AXES
    global SIM_ODE_M SIM_ODE_K SIM_ODE_C
    ...
    for i = 2:length(t)
        cla();
        plot2d(t(1:i), x(1, 1:i), style = color("magenta"));
        xtitle('Displacement Response (Animating)', 'Time (s)', 'Displacement (m)');
        drawnow();
        xpause(15000);
    end
endfunction
